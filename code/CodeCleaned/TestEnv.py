...
class TestEnv(Env):
    ...
    def step(self, action):     
        ...
        try:
            ...
            return self.actual_image, reward, done, inference_time
    ...
    #print_stats è un array [true, "path"]
    def close(self,print_stats=(False,"")):
        ...
        """
        right_guesses=0
        model_accuracies = []  # Lista per salvare le accuracy di ogni modello
        
        for index in range(len(self.models_name_to_dict)):
            
            # Stampa quante volte è stato scelto ogni modello
            print(str(self.models_name_to_dict[index])+" has been chosen: "+ str(self.track_actions[index]))
            
            # Stampa quante previsioni corrette ha fatto ogni modello
            correct_guesses = self.track_right_actions_per_model[index]
            print(str(self.models_name_to_dict[index])+ "right guesses: " + str(correct_guesses))
            
            # Calcola l'accuratezza per ogni modello
            accuracy_per_model = correct_guesses * 100 / self.track_actions[index] if self.track_actions[index] > 0 else 0
            print(f"Accuracy for {self.models_name_to_dict[index]}: {accuracy_per_model:.2f}%\n")
            
            model_accuracies.append(accuracy_per_model)
        ...
        acc=right_guesses*100/(self.frame_per_episode-1)
        print("Percentage: " + str(acc))
        
        """
        RETURN RIGHT GUESSES
        """
        if print_stats[0]:
            self.plot_stats(self.models_name_to_dict, self.track_actions, self.track_right_actions_per_model, self.frame_per_episode, print_stats[1])
            
        return acc, model_accuracies
