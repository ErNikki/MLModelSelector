...
class singleModelTrainEnv(Env):
    ...
    def test(self,env,model,episodes,device, actual_frame):#(env, model, episodes, render=False, device=device, context=""):
        ...
        right_guesses,episode=env.close()
        """
        if right_guesses*100>self.best_res:
                self.best_res=right_guesses*100
                target_path = os.path.join(MODEL_SAVE_PATH, f"{episode}_target_episode_{episode}.pth")
                print(f"Saving target weights at Episode {episode} ...")
