...
class _NetSelectorEnv(Env):
    ...
    def __init__(self, models, train_data_loader, device, frame_per_episode, train_mode=True, x2_train=False, env_path=Path("models/")):
        ...
        """
        if train_mode:
            #viene inizializzato ad ogni reset
            self.actual_iterable=None
        else:
            #in caso di test viene inizializzato una sola volta
    ...
    def step(self, action):     
        ...
        if not self.x2_train:
            ...
            try:
                ...
                return self.actual_image, rewards, done, 0                
            except:
                #else end episode and create new iterable to shuffle dataloader
                done=True
                self.actual_iterable=iter(self.train_data_loader)
